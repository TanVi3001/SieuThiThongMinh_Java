...
public class EmployeeView extends JPanel {
    ...
    // BIẾN LƯU MÃ NV GỐC KHI CẬP NHẬT
    ...
    private void loadAutoCompleteData() {
        if (cbSearch == null) return;
        employeeNameList.clear(); cbSearch.removeAllItems(); cbSearch.addItem(""); 
    ...
    private void initUI() {
        ...
        JLabel lblTitle = new JLabel("Hồ Sơ Nhân Viên"); lblTitle.setFont(new Font("Segoe UI", Font.BOLD, 26)); lblTitle.setForeground(textDark);
        JLabel lblSub = new JLabel("Quản lý thông tin và chức vụ nhân sự"); lblSub.setFont(new Font("Segoe UI", Font.PLAIN, 13)); lblSub.setForeground(textGray);
        titlePanel.add(lblTitle); titlePanel.add(lblSub);

        JPanel toolPanel = new JPanel(new FlowLayout(FlowLayout.RIGHT, 15, 10)); toolPanel.setOpaque(false);
        cbSearch = new JComboBox<>(); styleSearchBox(cbSearch);

        JPanel searchFieldWrapper = new JPanel(new BorderLayout(5, 0));
        searchFieldWrapper.setBackground(Color.WHITE); searchFieldWrapper.setPreferredSize(new Dimension(450, 45));
        ...
        JPanel centerPanel = new JPanel(new BorderLayout(25, 0)); centerPanel.setOpaque(false);
        RoundedPanel formCard = new RoundedPanel(20, cardWhite);
        formCard.setPreferredSize(new Dimension(360, 0)); formCard.setLayout(new GridBagLayout());
        ...
        txtName = createTextField("Nhập tên..."); txtPhone = createTextField("Nhập số điện thoại...");
        txtEmail = createTextField("Nhập email...");

        cbRole = new JComboBox<>(); styleComboBox(cbRole, "Nhập phân quyền...");
        for (String r : roleList) cbRole.addItem(r);
        ...
        JPanel genderPanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 15, 0)); genderPanel.setOpaque(false);
        ...
        JPanel btnGrid = new JPanel(new GridLayout(2, 2, 12, 12)); btnGrid.setOpaque(false);
    ...
    private void initEvents() {
            ...
            public void mouseClicked(MouseEvent evt) {
                ...
                if (row >= 0) {
                    ...
                    if (role.contains("ADMIN") || role.contains("MNG") || role.contains("Quản")) {
                        ...
                        tblEmployees.clearSelection(); clearForm(); return;
                    }
                    
                    // LƯU RAW ID VÀ HIỂN THỊ MÃ BỊ CHE TRÊN FORM
                    currentSelectedRawId = String.valueOf(tableModel.getValueAt(modelRow, 7));
                    ...
                    if (isActivated) {
                        txtEmail.setEnabled(false); txtEmail.setToolTipText("Tài khoản đã được cấp, không thể thay đổi Email.");
                    } else {
        ...
        btnAdd.addActionListener(e -> {
            ...
            if (isEmailDuplicate(emp.getEmail(), null)) {
                JOptionPane.showMessageDialog(this, "Email này đã được sử dụng cho một nhân viên khác!", "Trùng lặp dữ liệu", JOptionPane.WARNING_MESSAGE);
                txtEmail.requestFocus(); return;
            }
            if (isPhoneDuplicate(emp.getPhone(), null)) {
                JOptionPane.showMessageDialog(this, "Số điện thoại này đã được đăng ký cho người khác!", "Trùng lặp dữ liệu", JOptionPane.WARNING_MESSAGE);
            ...
            if (employeeSql.insert(emp) > 0) {
                SyncVersionDao.bumpVersion("EMPLOYEES"); RealtimeClient.send("EMPLOYEES_CHANGED");
                try { new ActivationTokenService().issueToken(emp.getEmployeeId()); } catch (Exception ex) { ex.printStackTrace(); }
                ...
                final String email = emp.getEmail(); final String name = emp.getEmployeeName(); final String code = actualToken; 
                new Thread(() -> {
                    boolean ok = business.service.EmailService.sendActivationEmail(email, name, code);
                    SwingUtilities.invokeLater(() -> {
                        if (ok) JOptionPane.showMessageDialog(this, "Thành công! Mã kích hoạt đã gửi tới mail: " + email);
                        else JOptionPane.showMessageDialog(this, "Hồ sơ đã lưu nhưng gửi mail thất bại.", "Lỗi Email", JOptionPane.WARNING_MESSAGE);
                    });
                }).start();

                refreshAllData(); clearForm();
            }
        });
        ...
        btnUpdate.addActionListener(e -> {
            ...
            if (displayedId.isEmpty() || displayedId.startsWith("Mã")) {
                JOptionPane.showMessageDialog(this, "Vui lòng chọn nhân viên trong bảng để cập nhật!"); return;
            }
            ...
            if (isEmailDuplicate(emp.getEmail(), idToUpdate)) {
                JOptionPane.showMessageDialog(this, "Email này đã bị trùng với một nhân viên khác!", "Trùng lặp dữ liệu", JOptionPane.WARNING_MESSAGE);
                txtEmail.requestFocus(); return;
            }
            if (isPhoneDuplicate(emp.getPhone(), idToUpdate)) {
                JOptionPane.showMessageDialog(this, "Số điện thoại này đã bị trùng với một nhân viên khác!", "Trùng lặp dữ liệu", JOptionPane.WARNING_MESSAGE);
                txtPhone.requestFocus(); return;
            }
            ...
            if (employeeSql.update(emp) > 0) {
                ...
                if (emailChanged && !isActivated) {
                    ...
                    new Thread(() -> {
                        ...
                        SwingUtilities.invokeLater(() -> {
                            if (ok) JOptionPane.showMessageDialog(this, "Đã cập nhật hồ sơ và gửi lại Mã Kích Hoạt mới tới:\n" + emailToSend);
                            else JOptionPane.showMessageDialog(this, "Cập nhật thành công nhưng gửi mail thất bại!", "Lỗi", JOptionPane.WARNING_MESSAGE);
                        });
                    }).start();
                } else {
        ...
        btnDelete.addActionListener(e -> {
            if (currentSelectedRawId.isEmpty()) return;
            if (JOptionPane.showConfirmDialog(this, "Xác nhận xóa hồ sơ này?", "Xác nhận", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
                if (employeeSql.delete(currentSelectedRawId) > 0) {
                    RealtimeClient.send("EMPLOYEES_CHANGED"); refreshAllData(); clearForm();
                }
            }
        });
    ...
    private Employee getEmployeeFromForm() {
        ...
        JTextField roleEditor = (JTextField) cbRole.getEditor().getEditorComponent(); String role = roleEditor.getText().trim().toUpperCase();

        if (name.isEmpty() || phone.isEmpty() || email.isEmpty() || gender.isEmpty() || role.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Vui lòng điền đầy đủ các thông tin cá nhân và chức vụ (*)"); return null;
        }
        if (!role.equals("R_STAFF_SALE") && !role.equals("R_STAFF_VIEW_PROD")) {
            JOptionPane.showMessageDialog(this, "Phân quyền không hợp lệ!\nQuản lý chỉ được phép cấp quyền:\n- R_STAFF_SALE\n- R_STAFF_VIEW_PROD", "Cảnh báo bảo mật", JOptionPane.WARNING_MESSAGE);
            return null;
        }
        ...
        if (!isValidPhone(phone)) {
            JOptionPane.showMessageDialog(this, "Số điện thoại không hợp lệ!", "Lỗi định dạng", JOptionPane.ERROR_MESSAGE); txtPhone.requestFocus(); return null;
        }

        Employee e = new Employee(); e.setEmployeeName(name); e.setPhone(phone); e.setEmail(email); e.setGender(gender); e.setRole(role); e.setRoleId(role);
        return e;
    }
    ...
    private void loadDataToTable() { updateTable(employeeSql.selectAll()); }
    ...
    private void updateTable(List<Employee> list) {
        ...
        for (Employee emp : list) {
            tableModel.addRow(new Object[]{
                ...
                emp.getRole(), 
                emp.getGender(),
                emp.getEmployeeId() // RawId
            });
        }
    }

    private String maskSensitiveInfo(String info) {
        if (info == null || info.isEmpty()) return "Chưa có dữ liệu";
        if (info.length() > 6) { 
            String visiblePart = info.substring(0, 6);
            StringBuilder hiddenPart = new StringBuilder();
            for (int i = 6; i < info.length(); i++) hiddenPart.append("*");
    ...
    private void styleComboBox(JComboBox<String> cb, String placeholder) {
        cb.setPreferredSize(new Dimension(280, 38)); cb.setFont(new Font("Segoe UI", Font.PLAIN, 14)); cb.setBackground(Color.WHITE); cb.setEditable(true);
        JTextField editor = (JTextField) cb.getEditor().getEditorComponent(); editor.putClientProperty("JTextField.placeholderText", placeholder);
    ...
    private JLabel createLabel(String text) {
        JLabel lbl = new JLabel(text); lbl.setFont(new Font("Segoe UI", Font.BOLD, 13)); lbl.setForeground(textDark); return lbl;
    }

    private JTextField createTextField(String placeholder) {
        JTextField txt = new JTextField(); txt.setPreferredSize(new Dimension(200, 38)); txt.setFont(new Font("Segoe UI", Font.PLAIN, 14));
    ...
    private JButton createCustomButton(String t, Color bg, Color fg, ImageIcon icon) {
        ...
        if (icon != null) btn.setIcon(new ImageIcon(icon.getImage().getScaledInstance(18, 18, Image.SCALE_SMOOTH)));
        btn.setFont(new Font("Segoe UI", Font.BOLD, 13)); btn.setForeground(fg); btn.setBackground(bg);
        btn.setPreferredSize(new Dimension(140, 45)); btn.setCursor(new Cursor(Cursor.HAND_CURSOR)); btn.setFocusPainted(false); btn.setBorderPainted(false); btn.setContentAreaFilled(false);
    ...
    private void setupTableStyle() {
        tblEmployees.setRowHeight(35); tblEmployees.setFont(new Font("Segoe UI", Font.BOLD, 14)); tblEmployees.setShowVerticalLines(false);
        tblEmployees.setSelectionBackground(new Color(237, 242, 255)); tblEmployees.setSelectionForeground(textDark); tblEmployees.getTableHeader().setReorderingAllowed(false);

        DefaultTableCellRenderer headerRenderer = new DefaultTableCellRenderer();
        headerRenderer.setBackground(bgLight); headerRenderer.setForeground(Color.BLACK); headerRenderer.setFont(new Font("Segoe UI", Font.BOLD, 14));
        headerRenderer.setHorizontalAlignment(JLabel.CENTER); headerRenderer.setBorder(BorderFactory.createEmptyBorder(10, 5, 10, 5));
        for (int i = 0; i < tblEmployees.getColumnModel().getColumnCount(); i++) tblEmployees.getColumnModel().getColumn(i).setHeaderRenderer(headerRenderer);

        DefaultTableCellRenderer customRenderer = new DefaultTableCellRenderer() {
            @Override public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
                Component c = super.getTableCellRendererComponent(table, value, isSelected, hasFocus, row, column);
                setHorizontalAlignment(JLabel.CENTER); String role = String.valueOf(table.getModel().getValueAt(table.convertRowIndexToModel(row), 5));
                if (role.contains("ADMIN")) { setBackground(isSelected ? new Color(248, 215, 218) : new Color(255, 235, 238)); setForeground(new Color(220, 53, 69)); }
                else if (role.contains("MNG")) { setBackground(isSelected ? new Color(212, 237, 218) : new Color(230, 245, 233)); setForeground(new Color(25, 135, 84)); }
                else { setBackground(isSelected ? new Color(237, 242, 255) : Color.WHITE); setForeground(isSelected ? textDark : Color.BLACK); }
                return c;
            }
        };
        for (int i = 0; i < tblEmployees.getColumnCount(); i++) tblEmployees.getColumnModel().getColumn(i).setCellRenderer(customRenderer);
    }

    private GridBagConstraints addGbc(GridBagConstraints gbc, int y, int b) { gbc.gridy = y; gbc.insets = new Insets(0, 0, b, 0); return gbc; }
    ...
    class RoundBorder implements javax.swing.border.Border {
        ...
        @Override public void paintBorder(Component c, Graphics g, int x, int y, int w, int h) {
            Graphics2D g2 = (Graphics2D) g.create(); g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
            g2.setColor(this.c); g2.setStroke(new BasicStroke(1.2f)); g2.drawRoundRect(x, y, w - 1, h - 1, r, r); g2.dispose();
        }
    ...
    private boolean isEmailDuplicate(String email, String excludeEmpId) { return employeeSql.existsByEmailGlobal(email, excludeEmpId); }
    private boolean isValidPhone(String phone) { return phone != null && !phone.isEmpty() && phone.matches("^(0)(3|5|7|8|9)[0-9]{8}$"); }
        ...
        try {
            ...
            for (Employee e : list) {
                if (e.getPhone() != null && e.getPhone().equals(phone)) {
                    if (excludeEmpId != null && e.getEmployeeId().equals(excludeEmpId)) continue;
                    return true;
                }
            }
