...
/**
 * Hộp thư thông báo – hiển thị chuông ở góc trên phải,
 * khi click sẽ mở popup danh sách thông báo.
 * Hỗ trợ real-time khi nhận event INVENTORY hoặc PRODUCTS.
 */
public class NotificationBell extends JPanel {
    ...
    public enum Audience { WAREHOUSE, MANAGER, SALE, ALL }
    ...
    public void addNotification(NotifItem.Type type, String title, String body, Audience target) {
        ...
        if (target != Audience.ALL && target != this.audience) return;

        String time = LocalTime.now().format(DateTimeFormatter.ofPattern("HH:mm"));
        NotifItem item = new NotifItem(type, title, body, time);

        // Tránh trùng lặp
        boolean exists = notifications.stream().anyMatch(n -> n.title.equals(title));
        if (exists) return;

        notifications.add(0, item); // mới nhất lên đầu
        unreadCount++;
        updateBadge();
        rebuildPopupList();
    }
    ...
    private void showPopup() {
        ...
        if (bellIcon != null) titleGroup.add(new JLabel(bellIcon));
        ...
        if (delIcon != null) { btnClear.setIcon(delIcon); btnClear.setIconTextGap(5); }
        ...
        btnClear.addActionListener(e -> { notifications.clear(); rebuildPopupList(); });
        ...
        if (x + 430 > screen.width) x = screen.width - 435;
        if (x < 0) x = 0;
    ...
    private JPanel createNotifCard(NotifItem n) {
        ...
        content.setBorder(new EmptyBorder(12, 14, 12, 16));
        ...
        if (typeIcon != null) iconCircle.add(new JLabel(typeIcon));
    ...
    if (cleanTitle.isEmpty()) cleanTitle = n.title;
    ...
    JLabel lblBody = new JLabel(
        "<html><div style='color:#555555; font-size:11px'>" + n.body + "</div></html>"
    );
    ...
    if (histIcon != null) timeRow.add(new JLabel(histIcon));
    ...
    public static class NotifItem {
        ...
        public final String title, body, time;
        public NotifItem(Type type, String title, String body, String time) {
